...
用netstat 也可以比较全面的 查看网络协议层丢包情况，下面我将其输出信息全部注释了下。
```shell
...
netstat 可以统计网卡环形缓冲区溢出，以及网卡发生错误的次数。
```shell
